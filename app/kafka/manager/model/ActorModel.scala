...
import kafka.manager.actor.cluster.{BrokerViewCacheActor, BrokerViewCacheActorConfig}
...
object ActorModel {
  ...
  case class TopicIdentity(topic: String,
   ...
   // logger.info("!!!!!!!! start sum offset for topic:" + topic)
    //val summedTopicOffsets: Long = getSumOffsetTopic(topic)
    //logger.info("!!!!!!!! sum offset for topic:" + getSumOffsetTopic(topic))
  ...
  def getSumOffsetTopic(topic: String): Long = {
    var sumOffset = 0l
    val brokerList = List(
      "kafka01.vimpelcom.ru",
      "kafka02.vimpelcom.ru",
      "kafka03.vimpelcom.ru",
      "kafka04.vimpelcom.ru",
      "kafka05.vimpelcom.ru",
      "kafka06.vimpelcom.ru",
      "kafka07.vimpelcom.ru",
      "kafka08.vimpelcom.ru",
      "kafka09.vimpelcom.ru",
      "kafka10.vimpelcom.ru",
      "kafka11.vimpelcom.ru",
      "kafka12.vimpelcom.ru",
      "kafka13.vimpelcom.ru",
      "kafka14.vimpelcom.ru",
      "kafka15.vimpelcom.ru")

    for (broker <- brokerList) {
      ///Future {
      val tryResult = KafkaJMX.doWithConnection(jmxHost = broker, jmxPort = 9998, jmxUser = None, jmxSsl = false, jmxPass = None
      ) {
        mbsc => sumOffset += getOffsetSumBroker(topic, mbsc)
      }
    }
    // val brokerViewCacheActor : ActorPath = context.actorOf(bvcProps,"broker-view").path
    // KafkaJMX.doWithConnection(jmxHost = "kafka09.vimpelcom.ru", jmxPort = 9998, jmxUser = None, jmxSsl = false, jmxPass = None)
    sumOffset
  }

  def getOffsetSumBroker(topic: String, mbsc: MBeanServerConnection): Long = {
    val opt = Some(topic)
    ...
    private[this] def getTopicPartitionIdentity(td: TopicDescription,
      ...
      partMap.map { case (partition, replicas) =>
        ...
        val previousPartitionOffsets: Option[PartitionOffsetsCapture] = tdPrevious.flatMap {
          ptd =>
            Await.ready(ptd.partitionOffsets, Duration.Inf).value.get match {
              case Success(offsetMap) =>
                Option(offsetMap)
              case Failure(e) =>
                None
