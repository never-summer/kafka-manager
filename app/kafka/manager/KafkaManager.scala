...
import javax.management.{InstanceNotFoundException, MBeanServerConnection, ObjectName}
...
import ActorModel._
import kafka.manager.jmx.{KafkaJMX, OSMetric}
...
class KafkaManager(akkaConfig: Config) extends Logging {
  ...
  def getSumOffsetTopic(cluster: String, topic: String): Long = {
    ...
    val brokerMap: concurrent.Map[Int, BrokerIdentity] = new ConcurrentHashMap().asScala
    //var brokerMap = new mutable.HashMap[Int, BrokerIdentity] with
    //mutable.SynchronizedMap[Int, BrokerIdentity]

    getBrokerList(cluster).map {
      brokerL =>
        brokerL.fold(
          ...
          brokerList => {
            brokerList.list.map {
              brokerIdent =>
                brokerMap.put(brokerIdent.id, brokerIdent)
            }
            logger.info("brokerMap:" + brokerMap)
            logger.info("brokerList:" + brokerList.list)
          }
        )
    }

    val sum = getTopicIdentity(cluster, topic).map {
      topicId =>
        topicId.fold(
          error => logger.error("error:" + error),
          topicIdentity => topicIdentity.partitionsIdentity.map {
            topicIdent => {
              val partnum = topicIdent._2.partNum
              logger.info("partnum:" + partnum)
              val leader = topicIdent._2.leader
              logger.info("leader:" + leader)
              val broker = brokerMap.get(leader)
              logger.info("brokerHost:" + broker)
              val tryResult = KafkaJMX.doWithConnection(jmxHost = broker.get.host, jmxPort = broker.get.jmxPort, jmxUser = None, jmxSsl = false, jmxPass = None
              ) {
                mbsc => {
                  sumOffset += getOffsetSumBroker(topic, mbsc, partnum)
        ...
        logger.info("sumOffset2:" + sumOffset)
    }
    //Future.sequence(cosumdTopicSummary).map(_.toMap)
    logger.info("sumOffset3:" + sumOffset)
    val result = Await.result(sum, 60.seconds)
    //logger.info("result3:" + result)
    sumOffset
  }

  def getOffsetSumBroker(topic: String, mbsc: MBeanServerConnection, partition: Int): Long = {
    val opt = Some(topic)
    val objName = new ObjectName(s"kafka.log:type=Log,name=LogEndOffset,topic=" + topic + ",partition=" + partition)

    val attributeList = mbsc.getAttributes(objName, Array("Value"))
    val offset = attributeList.get(0).toString.replaceAll("Value =", "").replaceAll("\\s+", "")
    logger.info("for topic:" + topic + ",attributeList:" + attributeList + ",  attributeList.get(0):" + offset)
    offset.toLong
  }

  //  def getOffsetSumBrokerAll(topic: String, mbsc: MBeanServerConnection): Long = {
  //    val opt = Some(topic)
  //    val objName2 = new ObjectName(s"kafka.log:type=Log,name=LogEndOffset,topic=" + topic + ",partition=*")
  //    logger.info("for topic:" + topic + ", ObjectName:" + objName2)
  //    import scala.collection.JavaConverters._
  //    var sum = 0l;
  //    try {
  ...
  //      }
  //    } catch {
  //      case err: Exception => logger.error("Got some other kind of exception:" + err)
  //    }
  //    sum
  //  }
